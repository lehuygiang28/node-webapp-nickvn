      ...
      .then(acc => {
        ...
        User.findById(_req.session.User._id)
          .then(user => {
            ...
            // let updatePuchased = new UserPuchased({
            //   user_id: user._id,
            //   product_puchased: [{
            ...
            //   }]
            // });
            // UserPuchased.findOneAndUpdate(filter, updatePuchased, {upsert: true})
            //   .then(() => { console.log(`Puchased: ${JSON.stringify(updatePuchased)}`)
            //   })
        ...
        // Send the error message to views
        sendMessage(_req, res, next, { success: true, message: 'Mua tài khoản thành công' });
        return res.redirect('/lien-minh/acc-lien-minh');
      })
      .catch(next);
  }
